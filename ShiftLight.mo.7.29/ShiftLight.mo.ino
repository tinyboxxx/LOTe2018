...
//  VARIABLES1- for use ;  w/ the strips and translated into 255 RGB colors
uint32_t color1;
uint32_t color2;
uint32_t color3;
uint32_t flclr1;
...
// CONFIGURATION FOR THE ROTARY ENCODER 旋转电位器设置
...
//SETUP TO CONFIGURE THE ARDUINO AND GET IT READY FOR FIRST RUN 连接电脑
void setup()
{
  Serial.begin(115200);
  Serial.println("SUES BAJA 2018 By HCX WYF");
  Serial.println("Booting!");

  //get stored variables 存储变量
  getEEPROM();
  check_first_run();
  ...
  //senseoption   1 = Interrupt (pin 2), 2 = FreqMeasure (pin 8) ,1打断 2测频率
...
void loop()
{
  ...
  //Poll the Button, if pushed, cue animation and enter menu subroutine 按钮进入菜单
...
{
  prev_menu = 2;
  //保留在菜单 this keeps us in the menu
  while (menuvar == 1)
  {
    ...
    if (prev_menu != rotaryval || menu_enter == 1 || menu_enter == 2)
    {
      ...
      case 1:                                                   //Adjust the global brightness   调整全局亮度
        SEG_WRITE[0] = (SEG_G);                                 // -
        ...
        while (menu_enter == 1)
        {
          ...
          // Serial.println(brightval);串行，输出亮度
        ...
        //dimPin
      case 2:                                                   //Adjust the global brightness DIMMER (when Pin 9 == HIGH)调整全局亮度的调光器
        SEG_WRITE[0] = (SEG_G);                                 // -
      ...
      case 3:                                                           // ACTIVATION RPM   活动时的最高转速
        SEG_WRITE[0] = (SEG_G);                                         // -
        ...
        while (menu_enter == 1)
        {
          ...
          if (coloradjust1 == -128)
          {
            activation_rpm = activation_rpm - 10;
          }
          if (coloradjust1 == 64)
          {
            activation_rpm = activation_rpm + 10;
      ...
      case 9: // Color Segmentation   颜色的分割
      ...
      case 10:                                                          // PIXEL ANIMATION MODE     像素动画模式
